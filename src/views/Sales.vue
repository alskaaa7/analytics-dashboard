<template>
  <div class="page dark-theme">
    ...
    <transition-group name="stagger-fade" tag="div" class="debug-info" v-if="error || (salesData.length === 0 && !loading)">
      <div v-if="error" key="error" class="error-message slide-in">
        <span class="icon">⚠️</span>
        {{ error }}
      </div>
      <div v-if="!loading && salesData.length === 0" key="no-data" class="no-data-message slide-in">
        <span class="icon">🔍</span>
        Данные не найдены. Проверьте параметры фильтрации.
      </div>
    </transition-group>

    <transition-group name="stats-slide" tag="div" class="stats" v-if="salesData.length > 0">
    ...
    <transition name="chart-scale" mode="out-in">
      <div class="charts-section" v-if="salesData.length > 0 && chartData.labels.length > 0" key="chart">
    ...
    <transition name="pagination-slide">
      <Pagination
        v-if="pagination && salesData.length > 0"
...
const salesData = computed(() => {
  ...
  if (apiData.value && apiData.value.sales) {
    return apiData.value.sales
  }
...
const uniqueProducts = computed(() => {
  const products = new Set(salesData.value.map(item => item.nm_id).filter(Boolean))
  return products.size
})

const avgSaleValue = computed(() => {
  return salesData.value.length > 0 ? Math.round(totalSales.value / salesData.value.length) : 0
})
...
const computedStats = computed(() => [
  ...
  {
    value: uniqueProducts.value,
    label: 'Уникальных товаров',
    icon: '🛍️'
  },
  {
    value: avgSaleValue.value.toLocaleString() + ' ₽',
    label: 'Средний чек',
    icon: '📊'
  }
])
...
const chartData = computed(() => {
  ...
  const salesByDate = salesData.value.reduce((acc, item) => {
    if (item.date) {
        ...
        tooltip: {
          ...
          callbacks: {
            label: function(context) {
              return `Продажи: ${context.parsed.y.toLocaleString()} ₽`
            }
          }
        }
      },
...
const tableColumns = [
  { key: 'date', title: 'Дата продажи', type: 'datetime' },
  ...
  { key: 'finished_price', title: 'Финальная цена', type: 'currency' },
  { key: 'for_pay', title: 'К оплате', type: 'currency' },
  { key: 'discount_percent', title: 'Скидка %', type: 'number' },
  { key: 'spp', title: 'СПП', type: 'number' },
  ...
  { key: 'sale_id', title: 'ID продажи', type: 'string' },
...
watch([salesData, chartCanvas, chartType], () => {
...
@keyframes titleGlow {
  ...
  to {
    text-shadow: 0 0 30px rgba(5, 150, 105, 0.8), 0 0 40px rgba(4, 120, 87, 0.6);
  }
}
